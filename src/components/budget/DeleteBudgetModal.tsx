...
import { ConfirmPassweordForm } from './ConfirmPassweordForm';

export default function DeleteBudgetModal() {
  const router = useRouter()
  const pathname = usePathname()
  const seasrchParams = useSearchParams()
  const deleteBudgetId = seasrchParams.get('deleteBudgetId')
  const show = deleteBudgetId ? true : false

  // const hideModal = new URLSearchParams(seasrchParams.toString());
